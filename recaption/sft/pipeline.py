...
def make_generation_messages(sample: SourceSample) -> list[dict[str, object]]:
    """Build generation messages using the default arxiv-style instruction.

    Prefer calling ``loader.build_generation_messages(sample)`` so loaders can
    override the prompt for dataset-specific strategies.
    """
...
def generate_candidates(
    ...
    *,
    ...
    sampling = loader.generation_sampling_params
    effective_temperature = temperature if temperature != 0.2 else sampling["temperature"]
    effective_top_p = top_p if top_p != 0.85 else sampling["top_p"]
    effective_max_tokens = max_tokens if max_tokens != 700 else sampling["max_tokens"]
    with output_path.open("a", encoding="utf-8", buffering=1) as handle:
        for batch_start in range(0, len(pending_records), batch_size):
            ...
            conversations = [loader.build_generation_messages(sample) for sample in samples]
            outputs = engine.chat(
                conversations,
            ...
            for sample, output_text in zip(samples, outputs):
                try:
                    examples = loader.process_generation_output(sample, output_text, generator_model=model_repo)
...
def get_pending_generation_records(
    ...
    *,
    ...
    completed_source_ids = sanitize_existing_candidates(
        ...
        prompt_version=loader.generation_prompt_version,
    )
