...
class BaseLoader(ABC):
    ...
    # --- Generation hooks (override in loaders with different prompt strategies) ---

    @property
    def generation_schema(self) -> dict:
        from sft_recaption.prompts import GENERATION_SCHEMA

        return GENERATION_SCHEMA

    @property
    def generation_prompt_version(self) -> str:
        from sft_recaption.config import GENERATION_PROMPT_VERSION
    ...
    def generation_sampling_params(self) -> dict:
        ...
        return {"temperature": 0.2, "top_p": 0.85, "max_tokens": 700}

    @property
    def limit_mm_per_prompt(self) -> dict[str, int] | None:
        """Override to set per-modality limits (e.g. multi-image few-shot loaders)."""
        return None

    def build_generation_instruction_text(self, sample: "SourceSample") -> str:
        from sft_recaption.prompts import build_generation_instruction
    ...
    def build_generation_messages(self, sample: "SourceSample") -> list[dict]:
        ...
        content.append({"type": "text", "text": self.build_generation_instruction_text(sample)})
        return [{"role": "user", "content": content}]

    def _normalize_payload(self, payload: Any) -> Any:
        from sft_recaption.pipeline import normalize_generation_payload

        return normalize_generation_payload(payload)

    def _build_candidates(self, sample: "SourceSample", payload: Any, *, generator_model: str) -> list:
        from sft_recaption.pipeline import build_examples_from_bundle

        return build_examples_from_bundle(sample, payload, generator_model=generator_model)
    ...
    def process_generation_output(
        ...
        *,
        ...
        """Parse model output and return CandidateExample objects.

        Default: JSON extraction → schema validation → candidate building.
        Override in loaders that use plain-text or non-JSON output formats.
        """
        import jsonschema
        ...
        jsonschema.validate(payload, self.generation_schema)
        return self._build_candidates(sample, payload, generator_model=generator_model)
