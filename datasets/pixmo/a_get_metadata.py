...
DATASET_DIR = Path("/path/to/data/vision-datasets/raw/sft/hf___allenai___pixmo-ask-model-anything/data")
OUTPUT_DIR = Path("/tmp/metadata/pixmo-ask-model-anything/parquet")
...
def iter_url_tables(parquet_paths):
    seen_urls = set()
...
def write_metadata(parquet_paths):
    ...
    with pq.ParquetWriter(temp_path, OUTPUT_SCHEMA) as writer:
        for table in iter_url_tables(parquet_paths):
