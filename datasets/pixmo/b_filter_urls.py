...
INPUT_DIR = Path("/tmp/metadata/pixmo-ask-model-anything/parquet")
OUTPUT_DIR = Path("/tmp/metadata/pixmo-ask-model-anything/filtered")
...
def filter_file(input_path, output_path, robots):
...
def main():
    ...
    robots = load_robots()
    kept, dropped = filter_file(INPUT_PATH, OUTPUT_PATH, robots)
