...
INPUT_DIR = Path(os.environ.get("METADATA_DIR", "/tmp/metadata/Recap-DataComp-1B/parquet"))
OUTPUT_DIR = Path(os.environ.get("FILTERED_DIR", "/tmp/metadata/Recap-DataComp-1B/filtered"))
...
def filter_file(input_path, robots):
    ...
    with pq.ParquetWriter(temp_path, OUTPUT_SCHEMA) as writer:
        for batch in parquet_file.iter_batches(batch_size=BATCH_SIZE, columns=["url", "caption"]):
            ...
            for url, caption in zip(data["url"], data["caption"]):
                if allowed_by_robots(url, robots):
...
def filter_file_worker(input_path):
    return filter_file(input_path, ROBOTS)
