...
def get_configs_to_process(dataset_name: str, subset_name: Optional[str]) -> List[Optional[str]]:
    """
...
def setup_http_retry_backend(
    ...
    print("  Retries on: connection errors, timeouts, HTTP 429/500/502/503/504")
...
def main():
    ...
    if args.subset_name is not None:
        if len(configs_to_download) == 1:
            print("Mode: Single configuration download")
        else:
