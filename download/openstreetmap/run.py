...
def sample_from_bounding_box(
    ...
    all_tiles = [(x, y) for x in range(min_x, max_x + 1) for y in range(min_y, max_y + 1)]
...
def main(
    ...
    sampled_tiles = sample_from_bounding_box(bbox=bounding_box, zoom=zoom, sample_ratio=sample_ratio)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch raster tiles from a tile server.")
    parser.add_argument(
        "--bbox",
        type=float,
        nargs=4,
