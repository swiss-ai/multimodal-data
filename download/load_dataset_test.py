...
Usage:
  python builder_loader_test.py \
      ...
      --cache-dir "/path/to/data/.../hf_datasets_cache" \
      --method builder_load

  HF_HUB_CACHE="/path/to/data/.../hf_hub_cache" python builder_loader_test.py \
...
def get_system_max_map_count() -> Optional[int]:
    ...
    Returns:
        Maximum number of memory mappings allowed, or None if unable to determine
    """
    try:
...
def get_builder_split_info(
    ...
    logger.info("=== Builder Split Info ===")
...
def check_memory_mapping_limits(split_info: Dict[str, Dict[str, int]], split: str) -> None:
    ...
    if max_map_count is None:
        logger.warning(
            ...
            f"If loading fails with 'too many open files' or similar errors, "
            f"increase vm.max_map_count: sudo sysctl -w vm.max_map_count=<value>"
        )
        return
    ...
    if max_map_count < num_shards:
        logger.warning(
            ...
            f"This may cause loading failures. "
            f"Increase vm.max_map_count: sudo sysctl -w vm.max_map_count={num_shards * 2}"
        )
    else:
        logger.info(
...
def load_hf_dataset(
    ...
    if method == "default":
        ...
        try:
            ...
            raise FileNotFoundError("Dataset not prepared. Use method='default' or prepare dataset first.") from e
...
def main():
    ...
    for idx, config_name in enumerate(configs_to_test, 1):
        ...
        try:
            # Get builder split info first (before loading dataset)
            ...
            # Check memory mapping limits
            check_memory_mapping_limits(split_info, args.split)
