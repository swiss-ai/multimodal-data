...
class MeditronAdapter(BaseDataset):
    ...
    def stream(self, logger, skip: int | None = None, batch_size: int = 1):
        ...
        with ThreadPoolExecutor(max_workers=self.decode_workers) as executor:
            for sample in self.dataset:
                ...
                images_bytes = [item["value"]["bytes"] for item in sample["modalities"] if item["type"] == "image"]
    ...
    def _decode_batch(
        ...
        batch = []
        futures = [executor.submit(_decode_images, images_bytes) for _, images_bytes, _ in pending]
        for (sample_id, _, conversations), future in zip(pending, futures):
            ...
            if n_failed:
                logger.warning(f"Sample {sample_id}: {n_failed}/{len(pil_images)} images failed to decode")
            if not valid:
                continue
