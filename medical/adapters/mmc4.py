...
class MMC4Adapter(BaseDataset):
    ...
    def get_raw_image_url(self, doc):
        return [img_meta.get("raw_url") for img_meta in doc.get("image_info", []) if img_meta.get("raw_url")]
                ...
                while not documents_exhausted and len(pending_tasks) < self.batch_size:
                    try:
                        ...
                        for url in doc_urls:
                            ...
                            counter += 1
                            if counter % 10000 == 0:
                                logger.debug(f"Dispatched {counter} fetch tasks, yielded {yield_count} images.")
