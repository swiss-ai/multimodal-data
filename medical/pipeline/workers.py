...
class WorkerPool:
    ...
    def process_batch(self, samples: list[Sample]) -> list[Sample]:
        ...
        sub_batches = [serialized[i : i + sub_batch_size] for i in range(0, len(serialized), sub_batch_size)]

        logger.debug(f"Processing batch of {len(samples)} samples in {len(sub_batches)} sub-batches")
