...
class HuggingFaceDatasetWriter(BaseWriter):
    ...
    def write_batch(self, samples: list[Sample]):
        ...
        image_samples = [s for s in samples if isinstance(s, (ImageSample, ImageTextSample))]
    ...
    def _ensure_writer_open(self):
        if self._writer is None:
            ...
            shard_path = os.path.join(self._data_dir, f"data-{self._shard_idx:05d}.arrow")
    ...
    def close(self):
        ...
        logger.info(f"Wrote {self._shard_idx} shards, {self._total_samples:,} samples, {self._total_bytes:,} bytes")
