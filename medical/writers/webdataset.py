...
def _ensure_img_tokens(text: str, n_images: int) -> str:
    ...
    missing = [f"<|img{i}|>" for i in range(1, n_images + 1) if f"<|img{i}|>" not in text]
    if missing:
        text = "".join(missing) + "\n" + text
    return text
...
class WebDatasetWriter(BaseWriter):
    ...
    def open(self, dataset_id: str):
        ...
        if start_shard > 0:
            logger.info(f"[{dataset_id}] Resuming from shard {start_shard} ({self._dataset_samples:,} samples)")
        else:
    ...
    def write_batch(self, samples: list[Sample]):
        ...
        for sample in samples:
            ...
            if isinstance(sample, MultiImageTextSample):
                ...
                record["txt"] = _ensure_img_tokens(sample.text, len(sample.images)).encode("utf-8")
